...
class Blockchain:
    ...
    def add_transaction(self, block_obj, role):
        blocks = self.find_chain(
            block_obj.bill_no, block_obj.state, block_obj.district)
        print(blocks)
        last_block = self.transactions.find_one(
            {"_id": {"$eq": ObjectId(blocks[-1])}})
        print(last_block)
    ...
    def find_chain(self, bill_no, state, district):
        chain = []
        trans = self.transactions.find_one(
            {"bill_no": {"$eq": bill_no}, "prev_hash": {"$eq": "-1"}})
        chain.append(str(trans["_id"]))

        if state != "NA":
            trans = self.transactions.find_one({"bill_no": {"$eq": bill_no}, "state": {
                                               "$eq": state}, "district": {"$eq": "NA"}})

            if trans:
                chain.append(str(trans["_id"]))

        if district != "NA":
            trans = self.transactions.find_one({"bill_no": {"$eq": bill_no}, "district": {
                                               "$eq": district}, "leaf": {"$exists": False}})
            if trans:
                chain.append(str(trans["_id"]))
    ...
    def get_full_chain_using_bill_no(self, bill_no):
        ...
        transs = self.transactions.find({"$and": [{"state": {"$ne": "NA"}}, {
                                        "bill_no": {"$eq": bill_no}}, {"district": {"$eq": "NA"}}]})
        states = []
        ...
        for i in transs:
            ...
            temp_list.append("")
            resp.append(temp_list)
            trans_dist = self.transactions.find({"bill_no": {"$eq": bill_no}, "state": {
                                                "$eq": i["state"]}, "prev_hash": {"$ne": "-1"}, "district": {"$ne": "NA"}})
            for d in trans_dist:
                print(d)
                temp_list = []
                temp_list.append({
                ...
                temp_list.append("")
                resp.append(temp_list)
                cont_trans = self.transactions.find({"bill_no": {"$eq": bill_no}, "leaf": {"$eq": True}, "state": {
                                                    "$eq": i["state"]}, "district": {"$eq": d["district"]}, "prev_hash": {"$ne": "-1"}})
                for c in cont_trans:
                    temp_list = []
                    if "status" in c.keys():
                        status = c["status"]
                    ...
                    temp_list.append({
                        ...
                        "f": str(c["purpose"])+"<br>"+str(c["amount"])+"<br>"+status
                    })
    ...
    def check_chain(self, block_obj):
        ...
        for i in range(len(blocks)-1, 0, -1):
            curr_block = self.transactions.find_one(
                {"_id": {"$eq": ObjectId(str(blocks[i]))}})
            prev_block = self.transactions.find_one(
                {"_id": {"$eq": ObjectId(str(blocks[i-1]))}})
